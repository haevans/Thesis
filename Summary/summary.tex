...
The possible values avaliable for the parameters $P$ and $S$ to take are shown in Figure~\ref{fig:NPcontss}, where the values are constrained from the measurements presented in this disseration and the results from the CMS collaboration in reference~\cite{Chatrchyan:2013bka}. The current measurements produce a cirular band of possible $P$ and $S$ values, and a precise measurement of \ADG through the \bsmumu \el will help resolve the ambiguities.


\begin{figure}[tbp]
    \centering
        \includegraphics[width=0.7\textwidth]{./Figs/Summary/}
