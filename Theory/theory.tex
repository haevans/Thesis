...
The decays can also proceed via Higgs-penguin diagrams however the contributions from these diagrams are negliable. %within the SM.
...
The difference in the coupling strengths is most easily illustrated through the Wolenstein parameterisation~\cite{} of the CKM matrix, which parameterises the matrix elements in terms of $\lambda = |V_{us}| \approx 0.22$. The CKM matrix then becomes
\begin{equation}
V_{CKM} =
 \begin{pmatrix}
...
The final source of supression of \bmumu decays comes from the helicities of the muons in the final state. The \bds is a spin zero particle and for angualar momentum to be comserved in the decay the spins of the two muons must be opositely aligned. This leads to the muons having oposite helicities. % Therefore the prodiction of one of the muons will always be supressed.
However the weak force only couple to left-handed particle states and right-handed anti-particle states. In the high energy limit where particles are massles, negaitve helicity states are equal to left-handed states and positive helicity states are equal to right-handed states.
Therefore if the muons were massless the weak interaction could only produce a $\mu^-$ and a $\mu^+$ with oposite helicities which cannot conserve angular momentum in \bmumu decays.
Muon are not massless therefore \bmumu decay can occur but are supressed because $m_{\mu} \ll M_{B_{(s)}$ leading to one of the helicity states of the muons being disfavoured.

Overall \bmumu decays are highly supressed within the framework of the SM compared to other decay modes of \bsd mesons. Therefore these decays offere excellend processes in which to search for New Physics (NP) phemonoma because the contribution of NP affects to these decay rates can be at a similar order of magnitude to those from the SM.

\section{\bmumu Branching Fraction}
\label{sec:BFdef}
The \BF of a particle decay offers an excellent ovservable through which predictions of the SM can be compared to measured values. The \bmumum \BF is defined as the fraction of the total number of \bsd particle that decay into two muons. It can be calculated from the decay rate, which is the probability per untu tie that a \bsd decays into two muons.

The SM predictions are calculated from the ``prompt'' decay rate that ignore any evolution with time of the \bsd particles. The \BFs are calcuated using 
\begin{equation}
...
The purely leptonic final state of \bmumu decays enable the computation of the transistion amplitude to be splot in two so that all uncertainties arising from the bound \bsd states are encompased into one final parameter, $F_{B_{(s}}}$ the hadronic decay factor. This leads to a theortically clean predition for the \BF.
...
\label{eq:BF_form}
\end{equation}
where $\theta_W$ is the weak mixing angle and $M_{W}$ the mass of the W boson. The \BF has been paramterised in terms of $\mathcal{C}_{10}^{SM}$, $P$ and $S$, where $\mathcal{C}_{10}^{SM}$ is the SM value of the operator $\mathcal{C}_{10}$ and 
\begin{equation}
...
\label{eq:S}
\end{equation}
In the SM $P=1$ and $S=0$, however the \BFs are paramterised in terms of $P$ and $S$ because NP models can alter their values. 

The dependance of the \BFs on $\mathcal{C}_{10}$ makes \bmumu decays one of the best decays to study this parameter and these decays are also very senstive to scale particles~\cite{}. The presence of scalar particles could increase the \BFs above the SM expectation through $\mathcal{C}_S^{(')}$ be producing $S>0$, and the contribution from scalar particles is not subject to helicity constraints. Pseudoscalar particles can either enhance or supree the \BFs compared to the SM prediction depending on how the values of $\mathcal{C}_P^{(')}$ interfer with $\mathcal{C}_10^{(')}$ in NP models.


As well as the individual \BFs of \bdmumu and \bsmumu decays the ratio of the two \BFs is also an intersting variable to test the SM and study NP models.The ratio of \BFs is
\begin{equation}
...
The obersved relationship $\Delta \Gamma \ll \Delta m$ as well as $\Gamma_{12} \ll M_{12}$ are used to seperate the expressions for $\Delta m$ and $\Delta \Gamma$ to give
\begin{equation}
...
\end{equation}
The values of $p$ and $q$ can also be realted to the measureable quantities and $\Gamma_{12}$ and $M_{12}$ by diagonalising $(\mathbf{M} - \frac{i}{2}\mathbf{\Gamma})$ to produce
\begin{align}
...
The necessary parameters used to describe the time evolution of \bsd and \barbsd states have now all been expressed in terms of measureable or predicatbale quantities therefore the time dependant decay rates can now be evalutaed. The decay rates can be expressed as
\begin{align}
...
Additionally NP models can move the \BF values and \ADG away from the SM predictions by providing new particles that can mediate the interaction. These new particles would change the Wilson coefficiencts included in the paramters $P$ and $S$. The dependance of the \BFs and \ADG on $P$ and $S$ are different as shown in Equations~\ref{} and~\ref{}, therefore NP models can influence the obeservables differently. The allowed values of \ADG and the ration of the measured and prompt SM prediction for $\mathcal{B}$(\bsmumu) are shown in Figure~\ref{fig:NPmodels} for situations where $S=0$, $P=1$, $P\pm S = 1$ and $\varphi_P, \varphi_S \in {0, \pi}$. It is clear that if NP effects are not revealed in the \BF measurements, they could still appear in \ADG. Furthermore in some scenarios \ADG is needed to resolve degeneracies that arise from information from the \BF alone.
\begin{figure}[htbp]
    \centering
        \includegraphics[width=0.49\textwidth]{./Figs/placeholder.jpeg}
