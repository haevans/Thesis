...
\end{tabular}
\vspace{0.7cm}
\caption{Mass and electric charge of quarks and leptons in the SM taken from reference~\cite{Olive:2016xmw}. For quarks and lepton, only particles are listed; anti-particles have the same mass and opposite electric charge as their corresponding particles. All masses listed are measured values or limits expect for the gluons and photon. In the SM the gluons and the photon do not couple to the Higgs field and are therefore massless. An upper limit has been placed on the photon mass at $1 \times 10^{-18}$~$e$V and gluon masses up to a few \mevcc are not excluded. The quark masses use the $\overline{\mathrm{MS}}$ renormalisation scheme and apart from the $t$ quark mass that comes from direct measurements.}
\label{tab:fermions}
\end{center}
\vspace{-1.0cm}
\end{table}
