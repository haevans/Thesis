...
\begin{figure}[tbp]
    \centering
     \includegraphics[width= 0.49 \textwidth]{./Figs/BFAnalysis/hidef_Fig14top.png}
     \includegraphics[width= 0.49 \textwidth]{./Figs/BFAnalysis/hidef_Fig14top.png}
     \caption{Power law fit to the resolution of quarkonia resonnances to determine mass resolution for \bdmumu and \bsmumu decays on Run~1 (left) and Run~2 (right) data.}
     \label{fig:interp}
\end{figure}
...
The particle identification and trigger efficiencies are different for \bdkpi and \bmumu decays. Therefore, the \bdkpi yields in each BDT bin are corrected for the different efficienies. %this. %for using by the different trigger and particle identification efficiencies. 
...
Therefore the mass \pdfs and expected yields of each background must be evaluated. The following sections summarise the information used for each background source in the \BF fit and Figure~\ref{} showns the mass distributions of the background sources.
