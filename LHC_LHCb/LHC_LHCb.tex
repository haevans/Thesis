...
The LHC accelerates the protons to the desired centre-of-mass energy using radio frequency cavities and guides them around the ring with superconducting dipole magnets. %I could add here some details about the magnets and how the LHC accelerates the protons.                                       
...
There are seven experiments on the LHC that study particles produced in proton and heavy ion collisions at the interaction points. These detecotors are: %. The main physics goals of these experiments are desribed briefly in the following:
\begin{itemize}
...
The amount of data recoreded by an experiment is given in terms of the time integrated luminosity.
...
The $b\bar{b}$ pair is created at the primary vertex of an event, where the protons collide. The quarks then hadronise to form form a range of \bhadrons, including $B^{+}$, $B^{0}_{s}$ and $\Lambda^{0}_{b}$. % a $b$ hadron. 
A typical $b$ hadron will exits for approximately 1.5 ps~\cite{Olive:2016xmw} before decaying into leptons, light hadrons or a mixtire of both. The vertex where the $b$ hadron decays is know as the secondary vertex and is displaced by XXX from the primary vertex. %The LHCb experiment was built to optimsed cost and physics output in the study of these decays. 
The LHCb experiment was desinged considering both the cost of the detector and the physics processes that could be studied with it. 
\begin{figure}[ptb] 
  \centering    
  \includegraphics[ width=0.45\textwidth]{./Figs/LHC_LHCb/07_rad_acc_scheme_right.pdf}%b_distrib_lhcb.pdf}
  \caption{Simulated angular distribution for \bbbar production at the LHC at center-of-mass energies of 7~\tev, angles are relative to the beam pipe with $\theta =0$ in the forward direction and $\theta = \pi$ in the backward direction. The red area showns \bbbar pairs that are produced within the angular coverage of the LHCb detector and the purple shaded area showns \bbbar pairs outside the detector coverage. Source: LHCb.}
...
The LHCb experiment is a single arm forward spectrometer, with an angular coverage of 10 to 300~mrad in the vertical direction and 10 to 250~mrad in the horizontal direction relative to the the beam pipe. %The angular coverage includes most b paris in one directs  sdfhjgajkfhdfsjfgf  was chosen to exploit the small angles at which \bbbar pairs are produced. 
A cross-section of the LHCb detector is shown in Figure \ref{fig:LHCb_detector}, where a right handed coordinate system is used. Protons collide at the interaction point on the left hand side of the diagram, a subset of particles produced during the collisions travel through the detector leaving information in the sub-detectors. %Most particles produced in the $pp$ collisions do not fall in the angular coverage of the LHCb experiment however
...
\label{VELO}
The VErtex LOcator (VELO)~\cite{Barbosa-Marinho:504321, LHCbVELOGroup:2014uea} is a silicon detector surrounding the interaction point. Its main purpose is to provide precise information about the $pp$ interaction vertices and secondary decay vertices of the particles produced. Information from the VELO enables precise measurements of particle lifetimes and the impact parameters of particle tracks necessary forthe study of particle decays. % physics analyses.
...
Excellent resolution of the position of each vertex is required in the VELO. To achieve this the sensors need to be as close as possible to the interaction point. This is acheived by making the VELO out of two retractable halves and including the $pp$ interaction point within the coverage of the VELO. 
...
\begin{figure}[tbp] 
  ...
  \caption{The resolution of the position of verticies in an event acheived by the VELO in 2012 data. The resolution is showns for primary vertcies perpendicular (left) and parallel (right) to the beam axis as a function of the number of tracks in an event~\cite{LHCbVELOGroup:2014uea}.}
...
\begin{figure}[tb] 
  ...
  \caption{The impact parameter resolution as a function of momentum (left) and inverse transverse momentum (right) acheived by the VELO for 2012 data~\cite{LHCbVELOGroup:2014uea}.}
  \label{fig:IP_res}
\end{figure}
...
\begin{figure}[tb]
  ...
  \caption{Magnet field of the dipole magnet along the length of the LHCb detector (left) and the layout to the LHCb detector \cite{Alves:2008zz}. The peak strength of the field occurs between the TT and T1-T3 station. }
...
The polarity of the magnetic field is periodically switched so that charged tracks are bend in the oposite direction. The polatity is switched to enable approximately equal amounts of data to be recorded for each magnet polarity during every year of data taking.
%so that it bends charged tracks in opposite directions. 
This is done to measure detection asymmetries in half of the detector and to help understand systematic uncertainties of $\mathcal{CP}$ violation measurements. % These are not relevant for B2mm.
...
The information left by the passage of charged particles through the VELO, TT and T stations is combined using track reconstruction algorithms to find trajectories of charged particles though the length of the LHCb detector and infer the particle momentum.  
...
The two RICH detectors provide identification information for different particle momentum ranges. The RICH-1 is senstive to particles in the momentum range of 2 to 40~GeV/c and the RICH-2 is sensitive to particles in the range 15~-~100~GeV/c, to acheive this the detectors are made of different materials.  
...
As well as ccovering different momentum ranges the RICH detectors also have different angular acceptances. The RICH-1 covers the full LHCb angular acceptance whereas the RICH-2 covers $\pm$~120~mrad in the horizontal and $\pm$ 100 mrad in the vertical direction. 
...
The expected Cherenkov angles for particle types with different momenta  is shown in Fgure~\ref{fig:RICH_radiator_predictions} for the radiator materials in the RICH detectors are shown in Figure~\ref{fig:RICH_preformance}.
The reconstructed Cherenkov angle as a function of momentum is shown in Figure~\ref{fig:RICH_preformance} for particles transversing the RICH-1 detector in 2011 data, the data showns distinct bands for each particle type as expected for the detector.
...
Compared to other particles muons have a high penetrating power due to their relatively large mass and because muons do not interact via the strong force, these properties are exploited in the muon detectors. There are five muon stations~\cite{Barbosa-Marinho:504326} located at the far end of the detector from the interaction point. %The information from these stations is used to identify muons that are used both in the trigger and in the analysis of particle decays in the data collected. %and is used in the to make decisions in the trigger.
...
There are three types of variables: the isMuon criteria, DLL variables; and ProbNN variables, which are describe in the following.
The performance of these variables at distinguishing between different particles types is described in references~\cite{Aaij:2014jba,Archilli:2013npa}
...
The information from all the PID detectors is combined using two different methods to provide global particle identification variables. One method is based on likelihood fits, producing the DLL variables, and the other is based on Neural Networks, producing ProbNN variables. In the first method, likelihood fits are performed in each sub-detector comparing charged particle tracks to different particle hypotheses. The resulting variable is the difference in the log-likelihoods between the track corresponding to a pion and a kaon, proton, muon or electron. The likelihood information form each sub detector is added linearly to form a combined likelihood. 
...
\begin{figure}[tbp] 
  \centering    
  \includegraphics[ width=0.49\textwidth]{./Figs/LHC_LHCb/hidef_Fig43left.png}
  \includegraphics[ width=0.49\textwidth]{./Figs/LHC_LHCb/hidef_Fig43right.png}
...
\begin{figure}[htb] 
  ...
  \caption{Diagrams of the trigger systems used in 2012 (left) and 2015 (right).}
...
The HLT1 is the first level of the HLT. It runs on the output of the L0 checking the decisions reducing the event rate. % from 1 MHz for processing in the HLT2. 
