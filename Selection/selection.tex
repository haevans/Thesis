...
In order to measure properites of \bmumu decays from data collected by the LHCb experiment, \bmumu decays must be sperated from backgrounds in the data. The main sources of backgrounds are described in Section~\ref{sec:backgroundoutline}.
...
\begin{table}[tbp]
\begin{center}
\begin{tabular}{p{0.19 \textwidth}p{0.30 \textwidth}p{0.14 \textwidth}p{0.13 \textwidth}p{0.11 \textwidth}}
\toprule
\toprule
...
The trigger, described in Section~\ref{Trigger}, selects events that could contain interesting particle decays. Candidates consistent with different particle decay hypothesis are reconstructed from events that were acceptance by the trigger.
...
\begin{figure}[tbp]
    \centering
        \includegraphics[width= 0.8 \textwidth]{./Figs/Selection/sel_eff_chart.pdf}
...
\begin{figure}[tbp]
   \centering
        \includegraphics[width=0.8\textwidth]{./Figs/Selection/Trigger.pdf}
...
\item impact paramter (IP) of the \bsd, this is the distance of closest approach of the $B$ to the primary vertex.
\end{itemize}
...
\begin{figure}[tbp]
 ...
 \includegraphics[width=0.49\textwidth]{./Figs/Selection/Min_IP_Mar.pdf}
    ...
    \caption{Distributions of input variables of the global BDT from simulated \bsmumu and \bbbarmumux decays used to train the global BDT passing cuts in Table~\ref{tab:BDTpresel}. Input variables are $\theta$ (a), \bs \chiIP (b), \bs \chivtx (c), $\sqrt{\Delta \phi^{2} + \Delta \eta^{2}}$ (d) and minimum muon \chiIP (e).}
    \label{fig:BDTvars}
\end{figure}
...
The performance of the BDTs is evaluated determining the signal significance for a range of cuts place on the output of each BDT. The signal significance is given by
\begin{equation}
...
The expected number of \bsmumu decays after different BDT cut values is straight forward to compute from the number in Table~\ref{tab:expectednumbers} because of the flattening procedure applied to the global BDT output and therefore the number of \bsmumu decays is evenly distributed across the BDT range. The number of combinatorial background decays expected after each BDT is determined from the number of decays in Table~\ref{tab:expectednumbers} and using information from simulated \bbbarmumux decays that have had all the \el selection requirement applied up until the cut on the global BDT. 
...
\begin{tabular}{lrrrr}
\toprule \toprule
Global BDT cut & $\mathcal{N}$(\bsmumu) & $R_{\epsilon}$ & $\mathcal{N}$(Combinatorial)  & $\lambda$ /$c^{2}$MeV$^{-1}$ \\ \midrule 
