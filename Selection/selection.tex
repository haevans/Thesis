...
\item parameters that dictate the BDT training - the training of a BDT is specified by several parameters; the number of trees, the tree depth, the minimum number of events a leaf can contain;, the `speed’ at which the boosting occurs and the number of cut values that a tree tries for a variable before making a decision.
\end{itemize}
...
The BDTS uses variables similar to those in the stripping selection to classify events;
\begin{itemize}
...
\end{itemize}
The BDTS is applied to all candidates passing the \bmumu and \bhh stripping lines, and candidates are required to have a BDTS value above 0.05. The chosen cut value has a efficiency of X $\%$ on \bsmumu decays and reject X $\%$ of \bbbarmumux decays. 
